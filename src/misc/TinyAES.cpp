...
QByteArray TinyAES::Encrypt(QByteArray b_input, QByteArray p_key, QByteArray p_iv)
{
    ...
    for (int part = 0; part < chunksCount; part++) {
        ...
        unsigned char * decrypted = (unsigned char *)malloc(inputSize);
        QByteArrayToUCharArray(p_chunk, decrypted);

        unsigned char * encrypted = (unsigned char *)malloc(inputSize); // encrypted text
...
QByteArray TinyAES::Decrypt(QByteArray b_input, QByteArray p_key, QByteArray p_iv)
{
    ...
    for (int part = 0; part < chunksCount; part++) {
        ...
        unsigned char * encrypted = (unsigned char *)malloc(inputSize);
        QByteArrayToUCharArray(p_chunk, encrypted);

        unsigned char * decrypted = (unsigned char *)malloc(inputSize); // decrypted text
